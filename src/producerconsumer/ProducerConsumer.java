...
class StackBasket {
	...
	/**
	 * show 生产方法. show 该方法为同步方法，持有方法锁； show 首先循环判断满否，满的话使该线程等待，释放同步方法锁，允许消费；
	...
	/**
	 * show 消费方法 show 该方法为同步方法，持有方法锁 show 首先循环判断空否，空的话使该线程等待，释放同步方法锁，允许生产； show
	 * 当不空时首先唤醒正在等待的生产方法，但是也只能让其进入就绪状态 show 等消费结束释放同步方法锁后生产才能持有该锁进行生产
	 * 
	 * @param b
...
class Consumer implements Runnable {
	...
	public void run() {
		for (int i = 0; i < 20; i++) {
			Mantou m = ss.pop();
			// System.out.println("消费了：---------" + m + " 共" + ss.index +
			// "个馒头");
